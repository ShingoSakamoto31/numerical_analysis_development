...
from cela3a_output import result
...
def process_copied_folder(copied_folder_path: Path) -> None:
    ...
    for idx, subfolder in enumerate(subfolders, start=1):
        ...
        filter_csv_path = subfolder / f"{subfolder_name}_A.csv"
        ...
        if not filter_csv_path.exists():
            print(f"\n警告: フィルターCSVファイルが見つかりません: {filter_csv_path}")
            continue
        ...
        try:
            ...
            sample_names = read_dict(sample_csv_path, "data", "sample")
            ...
            result = ParallelRunner(analyze, postprocess).run(
                ...
                data_for_enhancement=[
                    filter_by_entity(df_filter, entity="Surrounding Spots")
                ],
